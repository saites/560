...
s = ServerConnection('hydra14.eecs.utk.edu:6666')
assert(s.port_num == '6666')
assert(s.address == 'hydra14.eecs.utk.edu')

# check the expected values in the table
# note that this fails if inquire itself fails (that's a good thing)
for i in range(9):
    assert(s.server.inquire(1000+i) == (i+1)*1000)
assert(s.server.inquire(1010) == 0)

# now test inquire for a false value
assert(s.server.inquire(37) is None)

# check deposit and withdraw
for i in range(9):
    amt = randint(1,100)
    assert(s.server.deposit(1000+i,amt) is True)
    assert(s.server.inquire(1000+i) is (i*100)+amt)
    assert(s.server.withdraw(1000+i,amt) is True)
    assert(s.server.inquire(1000+i) is (i*100))

# check transfer
amt = randint(1,100)
amt1001 = s.server.inquire(1001)
amt1002 = s.server.inquire(1002)
assert(s.server.transfer(1001, 1002, amt) is (True, True))
assert(s.server.inquire(1001) + amt == amt1001)
assert(s.server.inquire(1002) - amt == amt1002)
assert(s.server.transfer(1002, 1001, amt) is (True, True))
assert(s.server.inquire(1001) == amt1001)
assert(s.server.inquire(1002) == amt1002)
assert(s.server.transfer(1001, 37, amt) is (True, False))
assert(s.server.transfer(37, 1001, amt) is (False, False))
